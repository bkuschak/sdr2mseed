 ...
 * 0 : SDR format
 *   ( this functionality is unused at the momemnt, perhaps forever )
 *
 ...
 ***************************************************************************/
int
decompressSDR (HeaderBlock *hblock, InfoBlock *iblock, int blocknum,
               int16_t *i16data)
{
  int bitCount       = 0;
 ...
 ***************************************************************************/
int
normalizeSDR24 (HeaderBlock *hblock, InfoBlock *iblock, int blocknum,
                int32_t *i32data)
{
  int samplesDecoded = 0;
  int byteCnt;
...
addnode (struct listnode **listroot, char *key, char *data)
{
  ...
  if (key)
    newlp->key = strdup (key);
  else
    newlp->key = key;
  if (data)
    newlp->data = strdup (data);
  else
    newlp->data = data;
  newlp->next   = 0;
